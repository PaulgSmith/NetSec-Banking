...
app.post("/login", function (req, res) {
  ...
  if (alphanumeric(username, "user") && alphanumeric(password, "password")) {
    let sql =
      `SELECT password ` +
      `FROM customer WHERE username = "${xssFilters.inHTMLData(
        req.body.username
      )}"`;
...
=======
  ...
  if (
    ...
    let sql =
      ...
      `VALUES ("${username}","${lastName}", "${firstName}", "${password}", "${address}")`;
...
app.get("/view_account", requireLogin, function (req, res) {
  ...
  connection.query(sql, function (err, result) {
    if (err) {
      ...
      // do a for loop to list all accounts:
      for (let account of result) {
        page += `<a href="/account_actions?number=${
          account[`account_number`]
        }" style="text-decoration: none;">${account[`account_number`]} - ${
...
=======
  ...
  if (
    ...
    let sql =
      ...
      `VALUES ("${accountName}", "${accountType}","${balance}", (SELECT username FROM customer WHERE username = "${req.session.user}"))`;
...
=======
  ...
  connection.query(sql, function (err, result) {
    if (err) {
      ...
      // do for loop to list all accounts
      for (let account of result) {
        page += `<option value="${account[`account_number`]}">${
          account[`account_name`]
        }</option>`;
      }
      page += `</select></li>`;
...
=======
  ...
  if (req.query.deposit) {
    if (checkNum(deposit, "float")) {
      sql = `UPDATE account SET balance = balance + "${deposit.toFixed(
        2
    ...
    if (checkNum(withdraw, "float")) {
      sql = `UPDATE account SET balance = balance - "${withdraw.toFixed(
        2
      )}"  WHERE account_number = "${account}" AND balance >= "${withdraw}"`;
    }
  }
  if (req.query.transfer) {
    if (checkNum(transfer, "float")) {
      let sql2 =
        `UPDATE account ` +
        `SET balance = balance - "${transfer.toFixed(2)}" ` +
        `WHERE account_number = "${account}" AND  balance >= "${transfer.toFixed(
          2
        )}";`;
      ...
      connection.query(sql2, function (err, result) {
        if (err) {
          ...
          if (result.affectedRows === 0) {
            ...
            sql =
              `UPDATE account SET balance = balance + "${transfer.toFixed(
                2
              )}" ` +
              `WHERE account_number = "${xssFilters.inHTMLData(
                req.body.accounts
              )}"; `;
            connection.query(sql, function (err, result) {
              if (err) {
